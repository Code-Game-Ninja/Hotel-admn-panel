...
import { Waves, Plus, Trash2, Pencil, X } from "lucide-react";
...
const EMPTY_FORM = { title: "", description: "", duration: "", price: 0, isActive: true };
...
export default function AdminSpaPage() {
    ...
    function openEdit(item: SpaTreatment) {
        setEditing(item);
        setForm({ title: item.title, description: item.description, duration: item.duration, price: item.price, isActive: item.isActive });
        setShowModal(true);
    }
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            if (editing) {
                await fetch(`/api/spa?id=${editing._id}`, {
                    method: "PUT",
    ...
    async function handleDelete(id: string) {
        ...
        await fetch(`/api/spa?id=${id}`, { method: "DELETE" });
        fetchItems();
    }

    async function toggleActive(item: SpaTreatment) {
        await fetch(`/api/spa?id=${item._id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-2xl font-semibold text-white tracking-tight">Spa Treatments</h1>
            ...
            {loading ? (
                ...
                <div className="bg-slate-900 border border-slate-800 p-10 text-center text-slate-500">
                    <Waves className="w-8 h-8 mx-auto mb-2 text-slate-600" />
                    <p>No spa treatments yet. Add your first treatment.</p>
                </div>
            ) : (
                    ...
                    {items.map((item) => (
                        <div key={item._id} className={`bg-slate-900 border p-6 ${item.isActive ? "border-slate-800" : "border-slate-800 opacity-60"}`}>
                            ...
                            <div className="flex items-center gap-4 text-sm mb-4">
                                <span className="text-amber-400 font-mono font-semibold">${item.price}</span>
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
                    <div className="bg-slate-900 border border-slate-700 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                        <div className="flex items-center justify-between p-6 border-b border-slate-800">
                            <h2 className="text-white font-semibold">{editing ? "Edit Treatment" : "Add Treatment"}</h2>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input value={form.duration} onChange={e => setForm({ ...form, duration: e.target.value })} placeholder="e.g. 90 min" required className="w-full bg-slate-800 border border-slate-700 text-white px-3 py-2 focus:outline-none focus:border-amber-500" />
                                </div>
                                <div>
                                    <label className="text-slate-400 text-xs uppercase tracking-wider block mb-1">Price (USD) *</label>
                                    <input type="number" value={form.price} onChange={e => setForm({ ...form, price: Number(e.target.value) })} min={0} required className="w-full bg-slate-800 border border-slate-700 text-white px-3 py-2 focus:outline-none focus:border-amber-500" />
                                </div>
                            </div>
